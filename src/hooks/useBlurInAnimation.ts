...
export const useBlurInAnimation = (duration: number = 3.5) => { // Fast, consistent fade-in to prevent flicker
  const [animationState, setAnimationState] = useState<BlurInAnimationState>({
    ...
    progress: 0,
    blurAmount: 12.0, // Reduced initial blur for faster, consistent fade-in
    opacity: 0.1, // Higher initial visibility to prevent flicker
  ...
  const updateAnimation = useCallback((currentTime: number) => {
    ...
    const blurAmount = Math.max(0, 12.0 * (1 - Math.pow(blurProgress, 0.8))); // Faster blur reduction from 12 to 0
    const opacity = Math.min(1.0, 0.1 + (0.9 * Math.pow(opacityProgress, 0.6))); // Quicker opacity progression
    const scale = 1.0; // No scaling - maintain position and size
    
    // Ensure smooth completion with coordinated timing
    const isStillAnimating = progress < 0.98; // Extended completion for ultra-smooth handoff
